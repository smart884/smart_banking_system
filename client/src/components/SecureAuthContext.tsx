...
interface AuthContextType {
  ...
  addRequest: (req: Omit<RequestItem, 'id' | 'status' | 'createdAt'>) => Promise<void>;
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
      ...
      if (user) {
        try {
          ...
          if (userDoc.exists()) {
            ...
            const profile = { ...DUMMY_USER, email: user.email || '', uid: user.uid };
            setUserProfile(profile);
            localStorage.setItem('sb_static_user', JSON.stringify(profile));
          }
        } catch (err) {
  ...
  const login = async (email: string, pass: string) => {
    try {
      ...
      if (userDoc.exists()) {
        ...
        let role: 'customer' | 'clerk' | 'manager' = 'customer';
  ...
  const addRequest = async (req: Omit<RequestItem, 'id' | 'status' | 'createdAt'>) => {
    try {
